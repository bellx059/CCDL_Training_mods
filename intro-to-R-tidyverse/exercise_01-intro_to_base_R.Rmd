...
str(metadata$data_row_count)
str(metadata$refinebio_accession_code)
...
head(metadata$refinebio_accession_code, 4)
```
...
brain_loc <- metadata$'brain location'
```
...
cerebral <- brain_loc == "Cerebral hemisphere"
```
