...
gene_df <- read_tsv("data/GSE19578.tsv")
```
...
metadata <- read_tsv("data/metadata_GSE19578.tsv")
```
...
metadata_clean <- metadata |> select(refinebio_accession_code, refinebio_age, refinebio_disease)
```
...
metadata_clean <- metadata_clean |> rename("sample_id" = "refinebio_accession_code",
                                            "age" = "refinebio_age",
                                            "disease" = "refinebio_disease")
```
...
metadata_clean <- metadata_clean |> mutate(disease = as.factor(disease))
str(metadata_clean)
```
...
summary(metadata_clean$disease)
```
...
metadata_clean$adult <- metadata_clean$age > 18
```
...
arrange(metadata_clean, desc(age))
```
...
write_tsv(metadata_clean, "results/cleaned_metadata_GSE19578.tsv")
```
